...
# Usage
if [ $# -lt 1 ]; then
    echo "Usage: $0 <vpc-id> [region] [<vpc-id> [region] ...]" >&2
    echo "" >&2
    echo "Examples:" >&2
    echo "  $0 vpc-12345678" >&2
    echo "  $0 vpc-12345678 us-east-1" >&2
    echo "  $0 vpc-11111111 us-east-1 vpc-22222222 us-west-2" >&2
    echo "  $0 vpc-11111111 vpc-22222222 us-west-2" >&2
    exit 1
fi
...
# Process arguments in pairs (vpc-id, optional region)
...
while [ $# -gt 0 ]; do
    ...
    # Check if next argument is a region or another VPC
    if [ $# -gt 0 ] && [[ ! $1 =~ ^vpc- ]]; then
        REGION=$1
        shift
    else
